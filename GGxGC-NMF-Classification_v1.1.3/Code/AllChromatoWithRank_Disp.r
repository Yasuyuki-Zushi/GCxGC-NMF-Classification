...
for (w2 in 1:lengthID ){
  ...
  text(RTini*2.2+Masking_left/60,MPeriod*0.85-Masking_top,paste("ID: ",w2,sep=""),cex=10,col="darkgray")
  ...
  text(RTini*2.4+Masking_left/60,MPeriod*0.65-Masking_top,paste("Rank: ",finalRank[w2],sep=""),cex=10,col=col.chosen)
...
windows()
par(mar=c(0,0,0,0))
par(mfrow=c(max(ceiling(length(var.sortID)/4),5),4))
