...
setwd(outputDrive)
suppressWarnings( dir.create("NMFinput_Unknowns_eachpic") )
...
StdValueList <- 0
        ...
        for (w in 1:length(unknown.sortID)){
        	...
        	fileroadedname <- paste("unkonwn",sortID[w],sep="")
        	cat(ReadFileName <- paste(fileroadedname, " <- as.numeric( scan('", filereadname,"'",", what='' ) )", sep=""))
        	eval(parse(text=ReadFileName ))
...
colnames(finalOutputbox) <- c("ID","File Name","Assigned Rank (Class)","Cosine Distance")
